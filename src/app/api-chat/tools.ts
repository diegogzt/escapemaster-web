...
export function getTools(token: string) {
  return {
    getDashboardSummary: tool({
      description:
        "Get dashboard summary including recent transactions, upcoming bookings, and occupancy alerts.",
      parameters: z.object({
    ...
    getRevenueStats: tool({
      description:
        "Get revenue and income stats for a given period (week, month, or year).",
      parameters: z.object({
        period: z
          .string()
          .describe(
            'Time period to query. Suggestion: "week", "month", "year".',
          ),
      }),
      // @ts-ignore
      execute: async ({ period }: { period: string }) => {
        ...
        if (p.includes("year") || p.includes("año") || p.includes("ano"))
          normalized = "year";
        return await fetchBackend(
          `/dashboard/stats?period=${normalized}`,
          token,
        );
      },
    }),
    getBookingsList: tool({
      description:
        "List bookings from the escape room. Supports filters: limit (max results), status (confirmed/pending/cancelled), dateFrom and dateTo (ISO date YYYY-MM-DD).",
      parameters: z.object({
        limit: z.number().optional().describe("Max number of results"),
        status: z
          .string()
          .optional()
          .describe("Status filter: confirmed, pending, cancelled"),
        dateFrom: z
          .string()
          .optional()
    ...
    getRoomsStatus: tool({
      description:
        "Get list of escape room rooms (games) with their status and price ranges.",
      parameters: z.object({
        _noop: z.string().optional(),
      }),
      // @ts-ignore
    ...
    getPaymentsLedger: tool({
      description:
        "List financial transactions (charges, payments, deposits). Use this for raw data CSV exports.",
      parameters: z.object({
        limit: z
          .number()
          .optional()
          .describe("Number of payments to fetch. Defaults to 100."),
      ...
      execute: async ({
        ...
        if (dateFrom) url += `&date_from=${dateFrom}`;
        if (dateTo) url += `&date_to=${dateTo}`;
        console.log("[tools] getPaymentsLedger called with:", {
          limit,
          dateFrom,
          dateTo,
        });
    ...
    getUsers: tool({
      description:
        "Get a list of all registered users in the platform, along with their assigned roles and status.",
      parameters: z.object({
        _noop: z.string().optional(),
      }),
      // @ts-ignore
    ...
    executeMathCalculator: tool({
      description:
        "A powerful calculator to perform complex math calculations. Pass a valid JavaScript math expression.",
      parameters: z.object({
        expression: z
          .string()
          .describe(
            'The robust mathematical expression to evaluate (e.g. "1500 * 0.2 + 45/2")',
          ),
      }),
      // @ts-ignore
