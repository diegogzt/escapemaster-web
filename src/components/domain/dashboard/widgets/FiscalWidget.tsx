...
export function FiscalWidget({
    ...
    const fetchData = async () => {
      try {
        ...
        if (result && result.breakdown && result.breakdown.length > 0) {
          ...
          setData({ breakdown: [], total_revenue: 0 });
        }
      } catch (error) {
  ...
  if (!data) {
    return (
      <div className={`bg-[var(--color-background)] p-6 rounded-2xl border border-[var(--color-beige)] shadow-sm h-full flex items-center justify-center`}>
        <p className="text-sm text-[var(--color-muted-foreground)]">No hay datos de facturación disponibles</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--color-muted-foreground)]">
            Evolución trimestral {new Date().getFullYear()}
          </p>
        </div>
      ...
      <div ref={containerRef} className="flex-1 h-[180px] w-full mt-2 relative">
        {dimensions.width > 0 && dimensions.height > 0 && hasData ? (
            ...
            <BarChart data={breakdown}>
              ...
              <Bar dataKey="amount" radius={[4, 4, 0, 0]}>
                {breakdown.map((entry: any, index: number) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={(entry.amount || 0) > 0 ? "#2c5f2d" : "#e5e7eb"}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-sm text-[var(--color-muted-foreground)]">Sin datos de facturación</p>
          </div>
        )}
      </div>
