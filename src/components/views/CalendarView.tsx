...
export function CalendarView() {
  ...
  const sessions = (calendarState.sessions || []).map((s) => ({
    ...s,
  ...
  const setCurrentDate = (d: Date) =>
    setCalendarState({ currentDate: d.toISOString() });
  const setSessions = (s: Session[]) =>
    setCalendarState({ sessions: s, lastFetched: Date.now() });
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      // Basic cache check
      const isFresh =
        calendarState.lastFetched &&
        Date.now() - calendarState.lastFetched < 60000;
      ...
      try {
        ...
        try {
          ...
          const roomsResult =
            roomsData?.rooms || (Array.isArray(roomsData) ? roomsData : []);
          roomsResult.forEach((room: any) => {
            roomsMap[room.id] = room;
          });
        } catch (rErr) {
          console.warn(
            "[Calendar] Failed to fetch rooms, colors might be affected",
            rErr,
          );
        }
        ...
        if (!Array.isArray(bookingsList)) {
          console.error(
            "[Calendar] API response bookings is not an array",
            bookingsList,
          );
        ...
        const transformedSessions: Session[] = bookingsList
          ...
          .map((b: any) => {
            ...
            if (b.room_id && roomsMap[b.room_id]?.status_colors?.[status])
              color = roomsMap[b.room_id].status_colors[status];
        ...
        if (status === 403) {
          setError(
            "Acceso denegado (403). Es posible que necesites permisos adicionales o que la sesión haya expirado.",
          );
        } else if (status === 401) {
        ...
        <div className="flex items-center gap-5">
          <div className="flex bg-[var(--color-light)]/50 rounded-2xl p-1.5 border border-beige/30">
            ...
            <button
              ...
              className="px-5 py-2 text-sm font-bold text-primary hover:bg-white rounded-xl transition-all"
            >
              Hoy
            </button>
            <button
            ...
            <div className="bg-[var(--color-background)] rounded-[2.5rem] border border-beige shadow-xl overflow-hidden">
              <div className="grid grid-cols-7 gap-px bg-beige/30">
                {[
                  ...
                  <div
                    key={d}
                    className="bg-[var(--color-background)] py-4 text-center"
                  >
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        day.isCurrentMonth &&
                        router.push(
                          `/bookings/create?date=${day.date.toISOString().split("T")[0]}`,
                        )
                      }
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span
                          className={`text-sm font-black w-8 h-8 flex items-center justify-center rounded-xl transition-all ${isTodayActive ? "bg-primary text-white shadow-lg shadow-primary/30" : "text-muted-foreground group-hover:text-primary"}`}
                        >
                          {day.date.getDate()}
                        </span>
                        {daySessions.length > 0 && (
                          <span className="text-[10px] font-bold text-muted-foreground opacity-40 px-1 bg-beige/30 rounded-md">
                            {daySessions.length}{" "}
                            {daySessions.length === 1 ? "Reserva" : "Reservas"}
                          </span>
                        )}
                      </div>
                          ...
                          >
                            <div className="flex justify-between items-center gap-1">
                              ...
                              <span className="opacity-60 shrink-0">
                                ...
                                {String(session.start.getMinutes()).padStart(
                                  2,
                                  "0",
                                )}
                              </span>
                            </div>
                            <div className="text-[9px] opacity-60 truncate font-medium">
                              {session.customer}
                            </div>
                          </div>
                        ))}
              ...
              <div className="grid grid-cols-[100px_repeat(7,1fr)] bg-beige/30 border-b border-beige/50">
                ...
                {Array.from({ length: 7 }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`bg-[var(--color-background)] py-4 text-center border-l border-beige/30 ${isTodayActive ? "bg-primary/5" : ""}`}
                    >
                      <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 mb-1">
                        {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"][i]}
                      </div>
                      <div
                        className={`text-lg font-black mx-auto w-9 h-9 flex items-center justify-center rounded-xl ${isTodayActive ? "bg-primary text-white shadow-lg shadow-primary/30" : "text-primary"}`}
                      >
                        {day.getDate()}
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-[100px_repeat(7,1fr)] min-h-full">
                  ...
                  <div className="flex flex-col">
                    {Array.from({ length: 24 }).map((_, i) => (
                      <div
                        key={i}
                        className="h-20 border-b border-beige/20 text-right pr-4 pt-2 text-[10px] font-bold text-muted-foreground/50"
                      >
                  ...
                  {Array.from({ length: 7 }).map((_, dayIndex) => {
                    ...
                    return (
                      <div
                        key={dayIndex}
                        className="relative border-l border-beige/30 group"
                      >
                        {Array.from({ length: 24 }).map((_, i) => (
                          <div
                            key={i}
                            className="h-20 border-b border-beige/10 hover:bg-primary/[0.02] transition-colors"
                          />
                        ))}
            ...
            <div className="bg-[var(--color-background)] rounded-[2.5rem] border border-beige shadow-xl overflow-hidden flex flex-col h-[700px]">
              <div className="p-6 bg-[var(--color-background)] border-b border-beige/50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center ${isSameDay(currentDate, new Date()) ? "bg-primary text-white shadow-xl shadow-primary/30" : "bg-primary/5 text-primary border border-primary/10"}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground font-medium">
                      {
                        sessions.filter((s) => isSameDay(s.start, currentDate))
                          .length
                      }{" "}
                      Reservas para hoy
                    </p>
                ...
                <div className="grid grid-cols-[100px_1fr] min-h-full">
                  <div className="flex flex-col bg-beige/5">
                    {Array.from({ length: 24 }).map((_, i) => (
                      <div
                        key={i}
                        className="h-24 border-b border-beige/20 text-right pr-6 pt-4 text-[11px] font-black text-muted-foreground/40"
                      >
                        {String(i).padStart(2, "0")}:00
                      </div>
                    ))}
                  ...
                  <div className="relative group">
                    {Array.from({ length: 24 }).map((_, i) => (
                      <div
                        key={i}
                        className="h-24 border-b border-beige/10 hover:bg-primary/[0.01] transition-colors"
                      />
                    ))}

                    {/* Sessions */}
                    {sessions
                      .filter((s) => isSameDay(s.start, currentDate))
                      .map((session, i, arr) => {
                        const startHour = session.start.getHours();
                        const startMin = session.start.getMinutes();
                        const duration =
                        ...
                        const overlapCount = arr.filter(
                          (s) => s.start < session.end && s.end > session.start,
                        ).length;
                        const overlapIndex = arr
                          .filter(
                            (s) =>
                              s.start < session.end && s.end > session.start,
                          )
                          .indexOf(session);
                        ...
                        return (
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              <span className="text-xs font-black uppercase tracking-widest bg-white/20 px-2 py-0.5 rounded-lg">
                                {session.start.getHours()}:
                                {String(session.start.getMinutes()).padStart(
                                  2,
                                  "0",
                              ...
                              <div className="p-1.5 bg-white/20 rounded-lg group-hover/session:rotate-45 transition-transform">
                                <Plus size={12} className="rotate-45" />
                              </div>
                            </div>
                            <h4 className="text-base font-black truncate mb-1">
                              {session.title}
                            </h4>
